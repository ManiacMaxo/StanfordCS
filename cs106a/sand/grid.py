...
class Grid(Generic[_T]):
    ...
    @staticmethod
    def build(lst: List[List[Union[_T, None]]]) -> "Grid[_T]":
        """
        Utility.
    ...
    def get(self, x: int, y: int) -> Union[_T, None]:
        """
        ...
        if error:
            ...
            raise RuntimeError(
                "out of bounds get({}, {}) on grid width {}, height {}".format(
                    x, y, self.width, self.height
                )
            )

    def set(self, x: int, y: int, val: Union[_T, None]):
        """
