...
GridType = Grid[Union[Literal["s"], Literal["r"]]]


def do_move(grid: GridType, x_from: int, y_from: int, x_to: int, y_to: int):
    """
...
def is_move_ok(grid: GridType, x_from: int, y_from: int, x_to: int, y_to: int):
    """
    ...
    """
    if not grid.in_bounds(x_to, y_to) or grid.get(x_to, y_to):
        # OOB or target not empty
        return False
...
def do_gravity(grid: GridType, x: int, y: int):
    """
...
def do_brownian(grid: GridType, x: int, y: int, brownian: int):
    """
...
def do_whole_grid(grid: GridType, brownian: int):
    """
...
def make_gui(top, width, height):
    ...
    scheck = tkinter.Checkbutton(
        top, text="Brownian", name="brownian", variable=brownian_on
    )
    scheck.grid(row=0, column=1, sticky="w")
    brownian_on.set(1)

    scale = tkinter.Scale(
        top, from_=0, to=100, orient=tkinter.HORIZONTAL, variable=brownian_val
    )
    ...
    fps_label = tkinter.Label(
        top, text="0", fg="lightgray"
...
def big_erase(grid, x, y, canvas):
    ...
    for ey in range(y1, y2 + 1):
        for ex in range(x1, x2 + 1):
            # circle around x,y
            if (
                grid.in_bounds(ex, ey)
                and abs(x - ex) ** 2 + abs(y - ey) ** 2 <= rad**2
            ):
                grid.set(ex, ey, None)
...
def main():
    ...
    grid: GridType = Grid(width, height)
